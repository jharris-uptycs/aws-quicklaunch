...
def stack_set_instance_exists(stack_set_name: str, account_id: str):
    ...
    """
    logger.info("Looking for stack instances of {} in account {}".format(stack_set_name, account_id))
    try:
        cfn_client = boto3.client("cloudformation")
        ...
        logger.info("stackset result {} for account_id {}".format(stack_set_result, account_id))
        if stack_set_result and "Summaries" in stack_set_result:
            ...
            while "NextToken" in stack_set_result:
                ...
                logger.info("Found {} instances of StackSet {}.".format(len(stack_set_list), account_id))
                logger.info("Expression {}".format(len(stack_set_list) > 0))
